...
export function SwapCard({ swapId, swap, userAddress, onRefresh }: { swapId: `0x${string}`; swap: any; userAddress: `0x${string}`; onRefresh?: () => void }) {
  ...
  return (
    <div className="rounded-2xl p-4 space-y-3 fade-up" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        {state === 1 && isCounterparty && <Button size="sm" onClick={() => setModal('fund')} loading={loading}>Fund (USDC)</Button>}
        {state === 2 && isInitiator && revealOpen && <Button size="sm" onClick={() => setModal('complete')}>Complete</Button>}
        {state === 2 && isCounterparty && revealOpen && <Button size="sm" variant="secondary" onClick={() => setModal('claim')}>Claim</Button>}
        {(state === 1 || state === 2) && (isInitiator || isCounterparty) && !revealOpen && <Button size="sm" variant="danger" onClick={() => setModal('refund')}>Refund</Button>}
        {state === 1 && isInitiator && timeLeftSec > 0 && <Button size="sm" variant="ghost" onClick={() => setModal('cancel')}>Cancel</Button>}
      </div>

      {modal === 'fund' && (
        <Modal title="Fund Swap" onClose={() => setModal(null)}>
          <p className="text-sm text-gray-400 mb-4">You will deposit <strong className="text-white">{formatUsdcAmount(BigInt(swap.usdcAmount) + BigInt(swap.bCollateral))} USDC</strong> (principal + 5% collateral).</p>
          <Button fullWidth onClick={() => tx(() => writeContractAsync({ address: CONTRACT_ADDRESS, abi: ABI, functionName: 'fund', args: [swapId] }), 'Funding...')} loading={loading}>Confirm Fund</Button>
        </Modal>
      )}
      {modal === 'complete' && (
        <Modal title="Complete Swap" onClose={() => setModal(null)}>
          ...
          <div className="mt-3">
            <Button fullWidth onClick={() => tx(() => writeContractAsync({ address: CONTRACT_ADDRESS, abi: ABI, functionName: 'complete', args: [swapId, (secretInput || savedSecret) as `0x${string}`] }), 'Completing...')} loading={loading}>Reveal & Complete</Button>
          </div>
        </Modal>
      )}
      {modal === 'claim' && (
        <Modal title="Claim Swap" onClose={() => setModal(null)}>
          ...
          <div className="mt-3">
            <Button fullWidth onClick={() => tx(() => writeContractAsync({ address: CONTRACT_ADDRESS, abi: ABI, functionName: 'claimAsCounterparty', args: [swapId, secretInput as `0x${string}`] }), 'Claiming...')} loading={loading}>Claim ETH</Button>
          </div>
        </Modal>
      )}
      {modal === 'refund' && (
        <Modal title="Refund Swap" onClose={() => setModal(null)}>
          ...
          <Button fullWidth variant="danger" onClick={() => tx(() => writeContractAsync({ address: CONTRACT_ADDRESS, abi: ABI, functionName: 'refund', args: [swapId] }), 'Refunding...')} loading={loading}>Confirm Refund</Button>
        </Modal>
      )}
      {modal === 'cancel' && (
        <Modal title="Cancel Swap" onClose={() => setModal(null)}>
          ...
          <Button fullWidth variant="danger" onClick={() => tx(() => writeContractAsync({ address: CONTRACT_ADDRESS, abi: ABI, functionName: 'cancel', args: [swapId] }), 'Cancelling...')} loading={loading}>Confirm Cancel</Button>
        </Modal>
      )}
    </div>
  )
